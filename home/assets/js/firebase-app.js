...
if (typeof firebase === 'undefined') {
    ...
    try {
        ...
        DatabaseInstance = {
            ...
            sendCommand: (command) => {
                const cmdRef = db.ref('home_system/commands').push();
                command.timestamp = firebase.database.ServerValue.TIMESTAMP;
                return cmdRef.set(command);
            },
            stopAI: () => {
                const cmdRef = db.ref('home_system/commands').push();
                return cmdRef.set({ type: 'stop', timestamp: Date.now() });
            },
            restartBridge: () => {
                const cmdRef = db.ref('home_system/commands').push();
                return cmdRef.set({ type: 'system_restart', mode: 'bridge', timestamp: Date.now() });
            }
        };
    } catch (e) {
