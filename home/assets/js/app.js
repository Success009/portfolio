...
const UI = {
    ...
    addLog(logData) {
        ...
        var label = logData.type.replace('ai-', '').toUpperCase();
        if (label === 'SYSTEM-MSG') label = 'SYS';
        entry.innerHTML = '<div class="meta"><span class="label">' + label + '</span><span class="ts">' + time + '</span></div><div class="msg">' + logData.message + '</div>';
        this.elements.logFeed.appendChild(entry);
        this.elements.logFeed.scrollTop = this.elements.logFeed.scrollHeight;
        if (this.elements.logFeed.children.length > 100) this.elements.logFeed.removeChild(this.elements.logFeed.firstChild);
    }
};
