...
function initDashboard() {
    ...
    if (btnTerminate) btnTerminate.onclick = () => {
        btnTerminate.textContent = "STOPPING...";
        btnTerminate.disabled = true;
        btnTerminate.style.opacity = "0.7";
        
        // Local Override: Force UI to IDLE immediately for snappiness
        UI.updateServerInfo({ ai_status: 'IDLE', current_turn: 0 });
        
        Database.stopAI().catch(e => {
...
async function executeAI() {
    ...
    try {
        if (p.startsWith('!')) {
            ...
            await Database.sendCommand({ type: 'exec_shell', payload: cmd });
            ...
            if (cmd === 'play') {
                await Database.sendCommand({ type: 'music_player', payload: `play|${args}` });
            } else if (cmd === 'stop') {
                await Database.sendCommand({ type: 'music_player', payload: 'stop' });
            } else if (cmd === 'restart') {
                restartBridge();
            } else if (cmd === 'help') {
                UI.appendLog({ type: 'info', message: "DIRECT COMMANDS:\n/play <song> - Play music\n/stop - Stop all media\n/restart - Reboot system\n! <cmd> - Run shell\n. <msg> - Direct TTS", timestamp: Date.now()/1000 });
            } else {
                UI.appendLog({ type: 'error', message: `Unknown command: ${cmd}`, timestamp: Date.now()/1000 });
            }
        } else {
            // AI Reasoning
