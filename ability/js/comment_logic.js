// Track active listeners
    ...
    window.getTotalCommentCount = (commentsObj) => {
        if (!commentsObj) return 0;
        let count = 0;
        Object.values(commentsObj).forEach(c => {
            count++; // Parent
            if (c.replies) count += Object.keys(c.replies).length; // Replies
        });
        return count;
    };
    ...
    window.loadComments = (key, containerOverride = null) => {
        const listId = containerOverride || `list-${key}`;
        ...
        ref.on('value', snap => {
            // Re-fetch because the DOM might have changed
            ...
            if(snap.exists()) {
                ...
                parents.forEach(p => {
                    ...
                    const isAdmin = p.ign === 'Admin';
                    const userColor = isAdmin ? '#e74c3c' : 'var(--primary)';
                    const editedTag = p.edited ? '<small style="opacity:0.5;margin-left:5px;">(edited)</small>' : '';
                    
                    div.innerHTML = `
                        <div class="comment-item">
                            <div class="comment-user" style="color:${userColor}">${escapeHtml(p.ign)}${editedTag}</div>
                    ...
                    p.repliesList.forEach(r => {
                        ...
                        const rIsAdmin = r.ign === 'Admin';
                        const rUserColor = rIsAdmin ? '#e74c3c' : 'var(--primary)';
                        const rEdited = r.edited ? '<small style="opacity:0.5;margin-left:5px;">(edited)</small>' : '';
                        const rDiv = document.createElement('div');
                        rDiv.className = 'comment-item reply-item';
                        rDiv.innerHTML = `
                            <div class="comment-user" style="color:${rUserColor}">${escapeHtml(r.ign)}${rEdited}</div>
