...
const cacheBlogPosts = () => {
    ...
    blogRef.once('value', (snapshot) => {
        if (snapshot.exists()) {
            ...
            localStorage.setItem('portfolio_blog_cache', JSON.stringify(posts.slice(0, 10)));
        }
    });
};
...
const loadBlogPosts = (containerId) => {
    ...
    // Helper to get the latest activity timestamp (creation or last update)
    const getLatestTimestamp = (post) => {
        let latest = post.timestamp || 0;
        if (post.updates) {
            Object.values(post.updates).forEach(up => {
    ...
    blogRef.on('value', (snapshot) => {
        ...
        // Sort by latest activity (Update timestamp or Creation timestamp)
        posts.sort((a, b) => getLatestTimestamp(b) - getLatestTimestamp(a));
        
        renderPostsToContainer(container, posts);
        
        // Update Cache for next visit
        localStorage.setItem('portfolio_blog_cache', JSON.stringify(posts));
    });
};
